...
function EditProfileForm() {
    ...
    var formSubmit = (e) => {
        e.preventDefault()
        axios.put("http://localhost:8000/accounts/profile/edit/", 
        uploadedAvatar ? {
            ...data,
            ...
            .catch((error) => {
                switch(error.response.status) {
                    ...
                    default:
                        ...
                        //     setFnameErrors([])
                        // }
                        error.response.data["phone_number"] ? setPhoneErrors(error.response.data["phone_number"]) : setPhoneErrors([])
                        error.response.data["first_name"] ? setFnameErrors(error.response.data["first_name"]) : setFnameErrors([])
                        error.response.data["last_name"] ? setLnameErrors(error.response.data["last_name"]) : setLnameErrors([])
                        error.response.data["avatar"] ? setAvatarErrors(error.response.data["avatar"]) : setAvatarErrors([])
                        setAlertData({
                            "variant": "danger",
            ...
            <form noValidate className="w-100" onSubmit={formSubmit}>
                <div className="row m-5">
                    <div className="col-lg-12 mb-5">
                        ...
                        <br />
                        <input type="file" id="addprofilepic" name="addprofilepic" accept="image/*" 
                            onChange={(e) => {
                    ...
                    <EditProfileTextField id="firstname" className="col-sm-6 mb-3" dataName="first_name" data={data} setData={setData} labelText="First Name" errors={fnameErrors}/>
                    <EditProfileTextField id="lastname" className="col-sm-6 mb-3" dataName="last_name" data={data} setData={setData} labelText="Last Name" errors={lnameErrors}/>
                    <EditProfileTextField id="phone" className="col-lg-12" dataName="phone_number" 
                        data={data} setData={setData} errors={phoneErrors} labelText="Phone Number" type="tel" 
                    />
